    ...
    <div className="space-y-10 animate-fadeIn min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="glass rounded-3xl p-8 bg-white border border-gray-100 relative overflow-hidden group">
          ...
          <p className="text-[10px] font-bold tracking-[0.1em] text-gray-400 mb-6 uppercase relative z-10">Active Agents</p>
          ...
          <div className="flex items-center gap-2 relative z-10">
            ...
            <p className="text-[9px] font-medium text-[#00E676] uppercase tracking-widest">Global Grid Active</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="absolute top-0 right-0 p-8">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Loka Revenue</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-gray-200" />
                <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Bank Baseline</span>
              </div>
            </div>
          </div>
          <div className="flex flex-col mb-10">
            <h3 className="text-xs font-black uppercase tracking-[0.2em] text-[#004D40] mb-2 italic">Cumulative Revenue</h3>
        ...
        <div className="glass rounded-[40px] p-10 bg-white border border-gray-100 shadow-sm">
          <h3 className="text-xs font-black uppercase tracking-[0.2em] text-[#004D40] mb-10 italic">Reserve Architecture</h3>
          <div className="h-[280px] relative mb-12">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              ...
              <span className="text-[9px] text-[#00E676] font-black uppercase tracking-[0.3em] mt-2">Dumbbell Ratio</span>
            </div>
          </div>
              ...
              <div key={item.name} className="p-4 bg-gray-50 rounded-2xl border border-gray-100 transition-all hover:border-black/10 group">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[11px] font-black text-black uppercase tracking-widest">{item.name.split(':')[0]}</span>
                  </div>
                  <span className="text-sm font-serif italic font-bold text-black">{item.value}%</span>
                </div>
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight ml-5">{item.name.split(':')[1]}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        <div className="glass rounded-[32px] p-6 bg-black text-white flex items-center justify-between col-span-1 shadow-xl border border-white/5">
          <div>
            <p className="text-[8px] font-black uppercase tracking-[0.3em] text-gray-500 mb-1">Security Audit</p>
            <p className="text-[11px] font-bold">Smart Contracts</p>
          </div>
          <div className="bg-[#00E676] text-black text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest">PASSED</div>
        </div>
      </div>
    </div>
  );
...
const StatCard: React.FC<{ title: string; value: string; trend: string; isVerified?: boolean }> = ({ title, value, trend, isVerified }) => (
  <div className="glass rounded-3xl p-8 bg-white border border-gray-100 hover:border-black/10 transition-all duration-500 group">
    <p className="text-[10px] font-bold tracking-[0.2em] text-gray-400 mb-6 uppercase">{title}</p>
    ...
    <div className="flex items-center gap-2">
      ...
      <p className={`text-[9px] font-black uppercase tracking-widest ${isVerified ? 'text-blue-600' : 'text-[#00E676]'}`}>
...
const TrustTag: React.FC<{ label: string; provider: string; status: string; time: string }> = ({ label, provider, status, time }) => (
  <div className="glass rounded-[32px] p-6 bg-white border border-gray-100 flex items-center justify-between hover:shadow-md transition-all shadow-sm">
    <div className="space-y-1">
      <p className="text-[8px] font-black uppercase tracking-[0.3em] text-gray-400">{label}</p>
      <p className="text-[11px] font-bold text-black">{provider}</p>
    </div>
    <div className="text-right">
      <div className="flex items-center gap-1.5 justify-end">
        <div className="w-1 h-1 rounded-full bg-[#00E676]" />
        <p className="text-[9px] font-black text-[#00E676] uppercase tracking-widest">{status}</p>
      </div>
      <p className="text-[8px] text-gray-300 font-bold uppercase tracking-tighter">{time}</p>
    </div>
  </div>
);
